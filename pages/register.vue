...
<div class="h-20 mb-6">
  <input v-model= "form.name.val"
      :class="{ 'border-red-500': form.name.errorMessage }" 
  type="text"
  class="block w-full py-3 px-4 appearance-none bg-gray-200 text-darkGray border rounded leading-tight focus:outline-none focus:bg-white" @blur = "validateName">
 ...
 data() {
  ...
  methods: {
    ...
    async upload({ localImageFile }){
      ...
      //ストレージオブジェクト作成
      const strageRef = this.$fireStorage.ref()

      //ファイルのパスを設定
      const imageRef = strageRef.child(
